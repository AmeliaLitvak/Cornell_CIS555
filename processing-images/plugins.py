...
def mono(image, sepia=False):
    ...
    for i in range(len(image)):
        for j in range(len(image[0])):
            brightness = int(0.3*image[i][j].red + 0.6*image[i][j].green \
            + 0.1*image[i][j].blue)
            if not sepia:    
                ...
                image[i][j].red = brightness
                image[i][j].green = int(0.6*brightness)
                image[i][j].blue = int(0.4*brightness)
    return True
...
def transpose(image):
    ...
    """
    # Change this to return True when the function is implemented
    imageT = np.transpose(image).tolist() 
    image.clear()      
    for i in range(len(imageT)):
        image.append(imageT[i])
    return True
